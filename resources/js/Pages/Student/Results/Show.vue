<!-- resources/js/Pages/Student/Results/Show.vue -->
<template>
  <StudentLayout>
    <div class="py-10">
...
// Recomendación calculada en el frontend
const recommendation = computed(() => {
  ...
  if (pct >= 90) return '¡Excelente! Tenés un dominio sólido del tema.';
  if (pct >= 80) return 'Buen resultado. Revisá las preguntas incorrectas para mejorar.';
  if (pct >= 60) return 'Estás cerca. Repasá las categorías con más errores y volvé a intentarlo.';
  return 'Necesitás reforzar el estudio. Revisá las explicaciones y volvé cuando estés listo.';
});
...
const retryExam = (shuffle) => {
  router.post(route('student.attempt.start', props.attempt.exam_id), { shuffle });
};
</script>